...
async def test_list_alerts_returns_tenant_configs():
    ...
    assert configs[0]["config"]["webhook_url"] == "…ack.com/services/abc"  # Redacted/masked URL assertion
...
async def test_create_alert_config():
    ...
    config_in = AlertConfigCreate(
        name="Ops Email",
        channel_type="email",
        config={"smtp_host": "smtp.gmail.com", "smtp_port": 587, "from": "hermes@ops.com", "to": "ops@ops.com", "password": "supersecret"},
        enabled=True
    )
    ...
    with patch("app.main.AlertConfig") as mock_alert_class:
        ...
        mock_alert_instance.to_dict.return_value = {
            ...
            "name": "Ops Email",
            "channel_type": "email",
            "config": {"smtp_host": "smtp.gmail.com", "smtp_port": 587, "from": "hermes@ops.com", "to": "ops@ops.com", "password": "••••••••"},
            "enabled": True
        }
...
async def test_update_alert_config_prevents_secret_overwrite():
    ...
    existing_config = AlertConfig(
        ...
        name="Ops Email",
        channel_type="email",
        config={"smtp_host": "smtp.gmail.com", "smtp_port": 587, "from": "hermes@ops.com", "to": "ops@ops.com", "password": "realpassword"},
        enabled=True
    )
    ...
    # Input sends redacted password placeholder back to api
    ...
    with patch("app.main.func") as mock_func:
        ...
        # Verify the actual password was kept as 'realpassword' rather than overwritten by placeholder
...
async def test_delete_alert_config():
    mock_db = AsyncMock()
    alert_id = uuid4()
    existing_config = AlertConfig(id=alert_id, tenant_id="test_tenant", name="Delete Me", channel_type="slack", config={})
...
async def test_test_alert_endpoint(mock_send_slack):
    ...
    result = await main_test_alert(alert_id=alert_id, tenant_id="test_tenant", db=mock_db)
